...
class SyncTcpConnection extends TcpConnection
{
    /**
    ...
    public function connect()
    {
        ...
        if ($this->transport !== 'unix') {
            ...
            if ($this->_contextOption) {
                ...
                $this->_socket = \stream_socket_client("tcp://{$this->_remoteHost}:{$this->_remotePort}",
                    $errno, $errstr, 0, \STREAM_CLIENT_CONNECT, $context);
            } else {
                $this->_socket = \stream_socket_client("tcp://{$this->_remoteHost}:{$this->_remotePort}",
                    $errno, $errstr, 0, \STREAM_CLIENT_CONNECT);
            }
        } else {
        ...
        // Check socket state.
        $address = \stream_socket_get_name($this->_socket, true);
        if (!$address) {
            $this->emitError(\WORKERMAN_CONNECT_FAIL, "cannot get peer name!");
            return;
        }
